    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            form.addEventListener("submit", function (event) {
                const isimInput = form.querySelector('input[name="isim"]');
                const mailInput = form.querySelector('input[name="mail"]');
                const telInput = form.querySelector('input[name="tel"]');
                const konuInput = form.querySelector('input[name="konu"]');
                const mesajInput = form.querySelector('textarea[name="mesaj"]');

                if (!isimInput.value || !mailInput.value || !telInput.value || !konuInput.value || !mesajInput.value) {
                    alert("Lütfen tüm alanları doldurun.");
                    event.preventDefault(); /
                    return;
                }

                if (telInput.value.length !== 10 || isNaN(telInput.value)) {
                    alert("Lütfen geçerli bir telefon numarası girin. (10 haneli, sadece rakamlar)");
                    event.preventDefault();
                    return;
                }
